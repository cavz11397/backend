...
public class QuestionRoute {
    ...
    @Bean
    public RouterFunction<ServerResponse> getOwnerAllQuestions(OwnerQuestionsListUseCase ownerQuestionsListUseCase) {
        return route(
                GET("/getOwnerAll/{userId}").and(accept(MediaType.APPLICATION_JSON)),
                request -> ServerResponse.ok()
                        .contentType(MediaType.APPLICATION_JSON)
                        .body(BodyInserters.fromPublisher(ownerQuestionsListUseCase.apply(
